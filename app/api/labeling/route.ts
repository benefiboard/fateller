...
//   "tweet_main": "원문의 핵심 주장과 중요 결론을 직접적으로 서술(추상적 설명이나 '~에 관한 글'이 아닌, 구체적 핵심 내용을 명확한 뜻을 나타내는 형식으로 서술). ",
//   "hashtags": ["핵심키워드1", "핵심키워드2", ... ], // 원문 내용에 따라 1~10개 사이로 필요한 만큼만 작성
//   "thread": [
...
// 기존 tweet_main 부분을 변경
// 내용에 맞게 구조화된 tweet_main 형식으로 수정
const TWITTER_PROMPT = `당신은 교과서나 학습자료 수준의 명확한 문서요약 전문가입니다.
1. 원문을 철저히 분석하여 가장 중요한 개념, 논점, 사례를 정확히 파악하세요.
2. 요약은 일타강사가 학생들에게 핵심을 설명하듯 명료하고 간결하게 작성하세요.
3. 독자가 한눈에 파악할 수 있도록 구조화하고 핵심 요점을 강조하세요.
...
결과는 반드시 유효한 JSON 형식으로만 응답하세요. 답변은 반드시 한글로 작성. 추가 설명이나 텍스트는 포함하지 마세요.`;
// const TWITTER_PROMPT = `당신은 교과서 수준의 명확한 문서요약 전문가입니다.
// 원문을 분석하여 중요 개념과 논점을 파악하고, 구조화된 요약을 JSON 형식으로 제공하세요.

// 다음 형식의 JSON 객체를 생성해주세요:
// {
//   "title": "원문의 핵심을 드러내는 제목",
//   "tweet_main": {
//     "sections": [
//       {
//         "heading": "주요 주제/영역",
...
//         ]
//       }
//     ] //sections는 1~10개 필요한 만큼
//   },
//   "hashtags": ["키워드1", "키워드2"], // 1~10개 필요한 만큼
//   "thread": [
//     "1. 핵심 개념 - 구체적 예시와 함께",
//     "2. 두 번째 개념 - 사례/데이터와 함께"
//     ...(필요한 만큼)
//   ], // 1~10개 필요한 만큼
//   "labeling": {
//     "category": "적합한 카테고리 하나",
//     "keywords": ["키워드1", "키워드2"], // 1~10개 필요한 만큼
//     "key_sentence": "핵심 메시지를 한 문장으로(명제형)"
//   }
// }
...
// 핵심 지침:
// 1. 교과서처럼 명확하게 서술
// 2. 원문 맥락에 맞게 구조화 (현황/문제점, 사례/요건, 해결방안 등)
// 3. 구체적 사례와 데이터 포함
// 4. 인과관계 명확히 표현
// 5. 완전한 문장 사용
// 6. 중요도 순으로 정보 제시

// 결과는 반드시 유효한 JSON 형식으로만 응답하세요. 한글로 작성하고 추가 설명은 포함하지 마세요.`;

const TIMEOUT_DURATION = 10500;
  ...
  try {
    ...
    try {
      ...
      if (data.usage) {
        ...
        const estimatedCost = (
          data.usage.prompt_tokens * 0.00001 +
          data.usage.completion_tokens * 0.00003
        ).toFixed(6);
