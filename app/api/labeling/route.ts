...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { originalTitle, originalImage, purpose = '일반', sourceId = null } = body;

    // text는 별도로 추출하고 제한할 새 변수 선언
    ...
    if (originalText.length > MAX_TEXT_LENGTH) {
      console.log(`텍스트 길이 제한 적용: ${originalText.length}자 → ${MAX_TEXT_LENGTH}자`);
    }
    ...
    console.log(`입력 텍스트 길이: ${processedText.length} 자`);
    console.log(`오리지널제목: ${originalTitle}`);
    console.log(`요청 의도: ${validPurpose}`);

    // 입력 텍스트 미리보기 (처음 100자)
    if (processedText.length > 100) {
      console.log(`입력 텍스트 미리보기: ${processedText.substring(0, 100)}...`);
    } else {
      console.log(`입력 텍스트: ${processedText}`);
    }
            ...
            },
            {
              ...
              content: originalTitle
                ? `오리지널 타이틀: "${originalTitle}" 
                   다음 텍스트를 분석하고 JSON 형식으로 응답해주세요: ${processedText}`
                : `다음 텍스트를 분석하고 JSON 형식으로 응답해주세요: ${processedText}`,
            },
          ],
