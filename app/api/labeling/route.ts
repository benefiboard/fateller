...
// 중요: 제공된 '오리지널 타이틀'과 원문의 '주요 사례와 예시'는 요약의 핵심 지침으로 반드시 최우선적으로 고려해야 합니다.
    ...
    "sections": [
      {
        ...
        "points": [
          "• 첫 번째 중요 포인트: 구체적이고 간결한 설명 + 관련 예시/사례",
          "• 두 번째 중요 포인트: 구체적이고 간결한 설명 + 관련 예시/사례"
        ]
      },
      {
        ...
        "sub_sections": [
          {
            ...
            "sub_points": [
              "◦ 세부 사항 1: 간결한 설명 + 구체적 예시",
              "◦ 세부 사항 2: 간결한 설명 + 구체적 예시"
            ]
          }
        ]
      },
      ... (필요한 갯수만큼 추가, sub_sections는 필요한 경우만 작성)
    ]
  },
...
3. 구체적 사례 강조: 
   - 원문에서 언급된 중요 사례, 데이터, 일화는 반드시 포함
   - 예시와 구체적 사례는 사람들의 기억에 오래 남으므로 우선적으로 포함
   - 원문에서 비유, 유명인 발언, 실생활 사례가 있다면 반드시 포함
...
사례 포함 특별 지침:
1. 원문에 나오는 모든 숫자 데이터(금액, 비율, 수치 등)는 특별히 중요한 정보로 간주하고 포함
2. 실제 인물, 기업, 제품 이름은 구체성을 높이므로 가능한 그대로 유지
3. 원문의 '이야기'나 '일화'는 전체 맥락을 유지하면서 간결하게 포함
4. 원문에서 강조된 비유나 은유는 그대로 유지하여 포함
5. 전문적 개념을 설명하는 실생활 예시는 반드시 포함
