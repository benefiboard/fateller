...
3. 섹션 구성 자유:
   ...
   - 원문에 맞지 않는 섹션은 포함하지 말고, 원문 구조에 맞는 새로운 섹션 추가 가능합니다
...
핵심 작성 지침:
1. 개인 메모 특성 존중하세요:
   - 원문의 구어체, 이모티콘, 줄임말, 강조 표현 등을 그대로 유지하세요
   - 개인적인 약어나 특수 표현도 원문 그대로 유지하세요
   - 불완전한 문장이나 메모 형식 텍스트도 그대로 반영하세요

2. 실용성 중심 접근하세요:
   - 계획, 할 일, 구매 목록 등 실행 가능한 항목은 명확하게 구분하세요
   - 날짜, 시간, 장소 등 중요 정보는 눈에 띄게 처리하세요
   - 원문에서 강조된 우선순위 표현 ("꼭!", "중요", "!!!" 등) 유지하세요

3. 간결성 유지하세요:
   - 원문이 짧은 경우 tweet_main은 1-2개 섹션으로 제한하세요
   - 각 섹션의 points는 원문 내용을 간결하게 요약하세요
   - thread는 원문이 매우 짧은 경우 1개 항목으로도 충분합니다

4. 개인 일상 관련 섹션 유연성:
   - 원문 내용에 가장 적합한 섹션 제목 선택하세요 (위 예시에 국한되지 않음)
   - 필요시 섹션 제목을 원문에 맞게 적절히 변경하세요
...
6. 매우 짧은 메모 처리하세요:
   - 원문이 한두 문장인 경우, 과도한 분석 없이 핵심만 간결히 요약하세요
   - 짧은 메모는 1개 섹션, 1-2개 points로 구성하세요
   - 필요한 경우 thread도 1개 항목으로 제한하세요
...
4. 학습 자료 특성 유지하세요:
   - 강의/수업 메타데이터(날짜, 교수명, 과목명 등) 보존하세요
   - 원문의 구조와 체계(장-절 구조, 번호 체계 등) 유지하세요
   - 원문의 강조점(★, ⭐ 등) 및 '중요하다'는 언급 유지하세요
   - 학생의 의문점이나 이해 어려움 표시("??", "이해 안됨" 등) 보존하세요

5. 시험 정보 우선화하세요:
   - '시험에 나온다', '예상문제', '자주 출제된다' 등의 언급은 반드시 포함하세요
   - 교수가 특별히 강조한 내용은 중요도 표시(★)와 함께 포함하세요
   - "시험에 나오지 않는다"는 정보도 명시적으로 포함하세요

6. 학습 계획 정보 보존하세요:
   - 다음 수업 준비사항, 과제, 읽기 자료 등이 원문에 있으면 반드시 포함하세요
   - 원문에 언급된 시험 일정, 범위 등도 정확히 포함하세요
