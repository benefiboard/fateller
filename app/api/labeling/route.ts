...
아래의 형식에 따라 정확히 JSON 객체를 생성해주세요:
{
  ...
  "labeling": {
    ...
    "key_sentence": "원문의 핵심을 담은 간결한 한 문장"
  }
}
...
const STUDY_PROMPT = `당신은 학습 필기 최적화 전문가입니다.
  ...
  "tweet_main": {
    "sections": [
      ...
      },
      {
        "heading": "시험 대비 정보",
        "points": [
          "• 출제 예상 내용: 원문에서 '시험에 나온다'고 언급된 정보",
          "• 시험 범위: 원문에 명시된 시험 범위"
        ]
      },
      {
        ...
        "points": [
          "• 과제: 원문에 언급된 과제",
          "• 읽기 자료: 원문에 언급된 참고문헌이나 페이지"
        ]
      }
    ]
  },
  "hashtags": ["과목명", "주요개념1", "주요개념2"],
  "thread": [
    "1. 핵심 주제 요약 - 가장 중요한 개념과 이론",
    "2. 주요 학습 포인트 - 시험과 이해에 필요한 요소",
    "3. 응용 및 활용 - 개념의 활용 방법(원문에 있는 경우)"
  ],
  "labeling": {
    "category": "가장 적합한 카테고리 한 가지만 선택",
    "keywords": ["키워드1", "키워드2"],
    "key_sentence": "학습 내용의 핵심을 한 문장으로 압축"
  }
}
...
핵심 작성 지침:
1. 학습자료 특성 강화:
   - 강의/수업 메타데이터 보존 (날짜, 교수명, 과목명 등)
   - 원문의 구조와 체계 유지 (장-절 구조, 번호 체계 등)
   - 원문의 강조점(★, ⭐ 등) 및 '중요하다'는 언급 유지
   - 학생의 의문점이나 이해 어려움 표시도 보존 ("??", "이해 안됨" 등)

2. 시험 정보 우선화:
   - '시험에 나온다', '예상문제', '자주 출제된다' 등의 언급은 반드시 "시험 대비 정보" 섹션에 포함
...
4. 완전한 정보 포함:
   - 원문의 모든 중요 내용은 누락 없이 포함 (특히 교수/강사가 특별히 언급/지시한 내용)
   - 원문 끝부분의 내용도 빠짐없이 포함 (종종 추가 과제나 중요한 보충 정보가 포함됨)
   - 주요 인물, 작품, 개념은 모두 포함 (이름만이라도 외우라고 한 내용 등)

5. 할루시네이션 방지:
   - 원문에 없는 내용은 절대 추가하지 말 것
   - 원문에 관련 섹션이 없으면 해당 섹션은 완전히 생략할 것 (억지로 채우지 말 것)

6. JSON 구조 유지:
   - 제시된 JSON 구조는 유지하되, 섹션 제목과 내용은 원문에 맞게 조정
   - 원문에 맞지 않는 섹션은 포함하지 말 것
...
최종 검증 체크리스트:
- 원문에서 교수/강사가 특별히 작성하도록 지시한 내용이 모두 포함되었는가?
- 시험 관련 언급(나온다/안나온다)이 모두 정확히 표시되었는가?
- 원문의 모든 주요 섹션/주제가 누락 없이 요약에 포함되었는가?
- 학생의 의문점이나 이해 어려움이 적절히 표시되었는가?
