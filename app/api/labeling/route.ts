...
아래의 형식에 따라 정확히 JSON 객체를 생성해주세요:
{
  ...
  "tweet_main": "원문의 핵심 주장과 중요 결론을 직접적으로 서술(추상적 설명이나 '~에 관한 글'이 아닌, 구체적 핵심 내용을 '~은 ~이다. 왜냐하면 ~이기 때문이다' 형식으로 명확하게 서술).  반드시 중요한 부분은 <hi>테그로 감싸서 중요함을 표현",
  "hashtags": ["핵심키워드1", "핵심키워드2", ... ], // 원문 내용에 따라 1~10개 사이로 필요한 만큼만 작성
  "thread": [
    "1. 가장 중요한 핵심 개념 - 구체적 예시와 함께 서술",
    "2. 두 번째 중요 개념 - 구체적 사례/데이터와 함께 서술",
    ... // 원문의 복잡도와 주요 개념 수에 따라 1~10개 사이로 필요한 만큼만 작성, 반드시 주요 개념마다 중요한 부분은 <hi>테그로 감싸서 중요함을 표현
  ],
  "labeling": {
